 ...
 */
export function validateConfig(config: Record<string, unknown>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  // Check severity
  if (config.severity !== undefined) {
    if (!['error', 'warning', 'suggestion'].includes(config.severity as string)) {
  ...
  if (config.output !== undefined) {
    if (!['text', 'json', 'github'].includes(config.output as string)) {
  ...
  if (config.aiProvider !== undefined) {
    if (!['openai', 'anthropic', 'gemini', 'qwen'].includes(config.aiProvider as string)) {
